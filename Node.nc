...
implementation {
   ...
   event void Boot.booted() {
      dbg(GENERAL_CHANNEL, "Node Booted - Link State Routing\n");
   ...
   event void AMControl.startDone(error_t err) {
      if (err == SUCCESS) {
         dbg(GENERAL_CHANNEL, "Radio On - Link State Routing\n");
         ...
         error_t lsrResult = call LinkStateRouting.start();
         if (lsrResult == SUCCESS) {
            dbg(GENERAL_CHANNEL, "Link State Routing successfully started\n");
         } else {
            dbg(GENERAL_CHANNEL, "Link State Routing failed to start. Error code: %d\n", lsrResult);
         }
   ...
   event message_t* Receive.receive(message_t* msg, void* payload, uint8_t len) {
      dbg(GENERAL_CHANNEL, "Packet received - Checking protocol\n");
      ...
      if (len == sizeof(pack)) {
         ...
         // Handle Link State Packets (PROTOCOL_LS)
            ...
            dbg(GENERAL_CHANNEL, "Link State Packet received - Calling handleLS()\n");
         ...
         // Handle other routing packets
            ...
            dbg(GENERAL_CHANNEL, "Routing packet - Calling routePacket()\n");
   ...
   event void CommandHandler.ping(uint16_t destination, uint8_t *payload) {
      dbg(GENERAL_CHANNEL, "Command Handler: PING received - Sending ping\n");
   ...
   event void CommandHandler.printNeighbors() {
      dbg(GENERAL_CHANNEL, "Command Handler: Printing neighbors\n");
      call NeighborDiscovery.printNeighbors();
   }

   // Command Handler: Print Routing Table
   event void CommandHandler.printRouteTable() {
      dbg(GENERAL_CHANNEL, "Command Handler: Printing routing table\n");
